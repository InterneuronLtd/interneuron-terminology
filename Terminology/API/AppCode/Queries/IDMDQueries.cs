...
namespace Interneuron.Terminology.API.AppCode.Queries
{
    public interface IDMDQueries
    {
        ...
        Task<List<T>> GetLookup<T>(LookupType lookType, bool ignoreCacheSource = false) where T: ILookupItemDTO;
        ...
        Task<List<DmdBNFCodeDTO>> GetAllBNFCodesFromDMD(bool ignoreCacheSource = false);

        Task<List<DmdATCCodeDTO>> GetAllATCCodesFromDMD(bool ignoreCacheSource = false);
        ...
        List<DmdSyncLog> GetDMDPendingSyncLogsByPagination(int pageNo, int pageSize);
    }
}
