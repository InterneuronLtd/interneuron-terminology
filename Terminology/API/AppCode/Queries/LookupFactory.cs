...
namespace Interneuron.Terminology.API.AppCode.Queries
{
    public class LookupFactory
    {
        ...
        public async Task<List<T>> GetLookupDTO<T>(LookupType lookupType, bool ignoreCacheSource = false) where T : ILookupItemDTO
        {
            var dtos = new List<T>();

            if (!ignoreCacheSource)
            {
                var lookupData = await CacheService.GetAsync<List<T>>(lookupType.GetCacheKeyByLookupTypeName());
            ...
            await CacheService.SetAsync(lookupType.GetCacheKeyByLookupTypeName(), dtos);
