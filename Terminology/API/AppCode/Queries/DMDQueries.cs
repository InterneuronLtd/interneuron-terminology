...
namespace Interneuron.Terminology.API.AppCode.Queries
{
    public partial class DMDQueries : IDMDQueries
    {
        ...
        public async Task<List<T>> GetLookup<T>(LookupType lookupType, bool ignoreCacheSource = false) where T : ILookupItemDTO
        {
            var lookupFactory = this._provider.GetService(typeof(LookupFactory)) as LookupFactory;

            var results = await lookupFactory.GetLookupDTO<T>(lookupType, ignoreCacheSource);
        ...
        public async Task<List<DMDDetailResultDTO>> GetDMDFullDataForCodes(string[] codes)
        {
            ...
            for (var reqIndex = 0; reqIndex < codes.Length; reqIndex += batchsize)
            {
                var batches = codes.Skip(reqIndex).Take(batchsize);
                batchedRequests.Add(batches.ToArray());
            }

            List<DMDDetailResultModel> resultsFromDB = new();
            
            foreach (var batches in batchedRequests)
            {
                var resultsVal =  await repo.GetDMDFullDataForCodes(batches);

                if (resultsVal.IsCollectionValid())
                    resultsFromDB.AddRange(resultsVal);
            }
        ...
        public List<DmdSyncLog> GetDMDSyncLogs()
        {
            ...
            var logs = repo.ItemsAsReadOnly.OrderBy(rec=> rec.SerialNum)?.ToList();
        ...
        public List<DmdSyncLog> GetDMDSyncLogsByPagination(int pageNo, int pageSize)
        {
            var repo = _provider.GetService(typeof(ITerminologyRepository<DmdSyncLog>)) as ITerminologyRepository<DmdSyncLog>;

            var logs = repo.ItemsAsReadOnly.OrderBy(rec => rec.SerialNum)?.Skip((pageNo - 1) * pageSize).Take(pageSize).ToList();
        ...
        public List<DmdSyncLog> GetDMDPendingSyncLogsByPagination(int pageNo, int pageSize)
        {
            var repo = _provider.GetService(typeof(ITerminologyRepository<DmdSyncLog>)) as ITerminologyRepository<DmdSyncLog>;

            var logs = repo.ItemsAsReadOnly
                .Where(rec => (rec.IsDmdUpdated != null && rec.IsDmdUpdated == true) && (rec.IsFormularyUpdated == null || rec.IsFormularyUpdated == false))?
                .OrderBy(rec => rec.SerialNum)
                .Skip((pageNo - 1) * pageSize)
                .Take(pageSize)
                .ToList();

            if (!logs.IsCollectionValid()) return new List<DmdSyncLog>();
        ...
        public List<DmdSyncLog> GetDMDPendingSyncLogs()
        {
            ...
            return logs.Where(rec => (rec.IsDmdUpdated.HasValue && rec.IsDmdUpdated == true) && rec.IsFormularyUpdated.HasValue == false || rec.IsFormularyUpdated == false)?
                .OrderBy(rec=> rec.SerialNum).ToList();
        }

        public async Task<List<DmdATCCodeDTO>> GetAllATCCodesFromDMD(bool ignoreCacheSource = false)
        {
            var atcDTOs = new List<DmdATCCodeDTO>();

            if (!ignoreCacheSource)
        ...
        public async Task<List<DmdBNFCodeDTO>> GetAllBNFCodesFromDMD(bool ignoreCacheSource = false)
        {
