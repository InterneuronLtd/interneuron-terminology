...
namespace Interneuron.Terminology.API.AppCode.Commands
{
    public partial class FormularyCommand : IFormularyCommands
    {
        ...
        private readonly IFormularyRepository<FormularyIngredient> _formularyIngredientRepository;
        private readonly IFormularyRepository<FormularyAdditionalCode> _formularyAdditionalCodeRepository;
        ...
        private Dictionary<string, string> _availrestrictLkp;
        private Dictionary<string, string> _basisOfNameLkp;
        private Dictionary<string, string> _controllerDrugCtgLkp;
        private Dictionary<string, string> _uomsLkp;
        private Dictionary<string, string> _doseFormsLkp;
        private Dictionary<string, string> _licAuthLkp;
        private Dictionary<string, string> _prescribingStsLkp;
        private Dictionary<string, string> _strengthsLkp;
        private Dictionary<string, string> _ingredientsLkp;
        private Dictionary<string, string> _routesLkp;

        public FormularyCommand(IServiceProvider provider, IConfiguration configuration, IMapper mapper, APIRequestContext requestContext, IFormularyQueries formularyQueries, IDMDQueries dMDQueries, IFormularyRepository<FormularyIngredient> formularyIngredientRepository, IFormularyRepository<FormularyAdditionalCode> formularyAdditionalCodeRepository)
        {
            _provider = provider;
            _mapper = mapper;
            ...
            _formularyIngredientRepository = formularyIngredientRepository;
            _formularyAdditionalCodeRepository = formularyAdditionalCodeRepository;
        }
        ...
        private async Task PrefillDMDLookup()
        {
            ...
            _availrestrictLkp = await _formularyQueries.GetLookup<DmdLookupAvailrestrictDTO, string, string>(LookupType.DMDAvailRestrictions, (rec) => rec.Cd.ToString(), rec => rec.Desc);

            _basisOfNameLkp = await _formularyQueries.GetLookup<DmdLookupBasisofnameDTO, string, string>(LookupType.DMDBasisOfName, (rec) => rec.Cd.ToString(), rec => rec.Desc);

            _controllerDrugCtgLkp = await _formularyQueries.GetLookup<DmdLookupControldrugcatDTO, string, string>(LookupType.DMDControlDrugCategory, (rec) => rec.Cd.ToString(), rec => rec.Desc);

            _uomsLkp = await _formularyQueries.GetLookup<DmdLookupUomDTO, string, string>(LookupType.DMDUOM, (rec) => rec.Cd, rec => rec.Desc);

            _doseFormsLkp = await _formularyQueries.GetLookup<DmdLookupDrugformindDTO, string, string>(LookupType.DMDDoseForm, rec => rec.Cd.ToString(), rec => rec.Desc);

            _licAuthLkp = await _formularyQueries.GetLookup<DmdLookupLicauthDTO, string, string>(LookupType.DMDLicensingAuthority, rec => rec.Cd.ToString(), rec => rec.Desc);

            _prescribingStsLkp = await _formularyQueries.GetLookup<DmdLookupPrescribingstatusDTO, string, string>(LookupType.DMDPrescribingStatus, rec => rec.Cd.ToString(), rec => rec.Desc);

            _strengthsLkp = await _formularyQueries.GetLookup<DmdLookupBasisofstrengthDTO, string, string>(LookupType.DMDPharamceuticalStrength, rec => rec.Cd.ToString(), rec => rec.Desc);

            _ingredientsLkp = await _formularyQueries.GetLookup<DmdLookupIngredientDTO, string, string>(LookupType.DMDIngredient, rec => rec.Isid.ToString(), rec => rec.Nm);

            _routesLkp = await _formularyQueries.GetLookup<DmdLookupRouteDTO, string, string>(LookupType.DMDRoute, rec => rec.Cd, rec => rec.Desc);
        }
        ...
        private async Task PrefillAdditionalCodesForDMDCodes(List<DMDDetailResultDTO> dmdResults)
        {
            ...
            dmdCodes.AsParallel().Each(rec =>
            {
                //MMC-509 - Cd should be there
                var atcForDMD = dmdATCCodes?.Where(atc => atc.DmdCd == rec && atc.Cd.IsNotEmpty()).ToList();

                //MMC-509 - Cd should be there
                var bnfsForDMD = dmdBNFCodes?
                .Where(bnf => bnf.Cd.IsNotEmpty() && bnf.DmdCd == rec)?
                .Select(rec => new DmdBNFCodeDTO { Cd = rec.Cd.Length >= 7 ? rec.Cd.Substring(0, 7): rec.Cd, DmdCd = rec.DmdCd, Desc = rec.Desc })?
        ...
        private void OverwriteFromExisting(DMDDetailResultDTO resDTO, FormularyHeader formularyHeader, List<FormularyHeader> existingFormulariesForCodes, IRepository<FormularyHeader> formularyHeaderRepo)
        {
            ...
            {
                //MMC-477: Import itself is not in use - moved to windows job
                //newVersionedFormulary = existingFormulary.CloneFormulary(_mapper, Guid.NewGuid().ToString());
                newVersionedFormulary = existingFormulary.CloneFormularyV2(_mapper);
        ...
        private void UpdateFormularyDetailDMDLookup(FormularyDetail formularyDetail)
        {
            if (formularyDetail == null) return;

            formularyDetail.SupplierName = (_supplierCodeNames.IsCollectionValid() && formularyDetail.SupplierCd != null && _supplierCodeNames.ContainsKey(formularyDetail.SupplierCd)) ? _supplierCodeNames[formularyDetail.SupplierCd] : formularyDetail.SupplierName;

            formularyDetail.RestrictionsOnAvailabilityDesc = (_availrestrictLkp.IsCollectionValid() && formularyDetail.RestrictionsOnAvailabilityCd != null && _availrestrictLkp.ContainsKey(formularyDetail.RestrictionsOnAvailabilityCd)) ? _availrestrictLkp[formularyDetail.RestrictionsOnAvailabilityCd] : formularyDetail.RestrictionsOnAvailabilityDesc;

            formularyDetail.BasisOfPreferredNameDesc = (_basisOfNameLkp.IsCollectionValid() && formularyDetail.BasisOfPreferredNameCd != null && _basisOfNameLkp.ContainsKey(formularyDetail.BasisOfPreferredNameCd)) ? _basisOfNameLkp[formularyDetail.BasisOfPreferredNameCd] : formularyDetail.BasisOfPreferredNameDesc;

            formularyDetail.ControlledDrugCategoryDesc = (_controllerDrugCtgLkp.IsCollectionValid() && formularyDetail.ControlledDrugCategoryCd != null && _controllerDrugCtgLkp.ContainsKey(formularyDetail.ControlledDrugCategoryCd)) ? _controllerDrugCtgLkp[formularyDetail.ControlledDrugCategoryCd] : formularyDetail.ControlledDrugCategoryDesc;

            formularyDetail.DoseFormDesc = (_doseFormsLkp.IsCollectionValid() && formularyDetail.DoseFormCd != null && _doseFormsLkp.ContainsKey(formularyDetail.DoseFormCd)) ? _doseFormsLkp[formularyDetail.DoseFormCd] : formularyDetail.DoseFormDesc;

            formularyDetail.FormDesc = (_formCodeNames.IsCollectionValid() && formularyDetail.FormCd != null && _formCodeNames.ContainsKey(formularyDetail.FormCd)) ? _formCodeNames[formularyDetail.FormCd] : formularyDetail.FormDesc;

            formularyDetail.CurrentLicensingAuthorityDesc = (_licAuthLkp.IsCollectionValid() && formularyDetail.CurrentLicensingAuthorityCd != null && _licAuthLkp.ContainsKey(formularyDetail.CurrentLicensingAuthorityCd)) ? _licAuthLkp[formularyDetail.CurrentLicensingAuthorityCd] : formularyDetail.CurrentLicensingAuthorityDesc;

            formularyDetail.PrescribingStatusDesc = (_prescribingStsLkp.IsCollectionValid() && formularyDetail.PrescribingStatusCd != null && _prescribingStsLkp.ContainsKey(formularyDetail.PrescribingStatusCd)) ? _prescribingStsLkp[formularyDetail.PrescribingStatusCd] : formularyDetail.PrescribingStatusDesc;

            formularyDetail.UnitDoseUnitOfMeasureDesc = (_uomsLkp.IsCollectionValid() && formularyDetail.UnitDoseUnitOfMeasureCd != null && _uomsLkp.ContainsKey(formularyDetail.UnitDoseUnitOfMeasureCd)) ? _uomsLkp[formularyDetail.UnitDoseUnitOfMeasureCd] : formularyDetail.UnitDoseUnitOfMeasureDesc;

            formularyDetail.UnitDoseFormUnitsDesc = (_uomsLkp.IsCollectionValid() && formularyDetail.UnitDoseFormUnits != null && _uomsLkp.ContainsKey(formularyDetail.UnitDoseFormUnits)) ? _uomsLkp[formularyDetail.UnitDoseFormUnits] : formularyDetail.UnitDoseFormUnitsDesc;
        }

        private void UpdateFormularyExcipientDMDLookup(FormularyExcipient excipient)
        {
            if (excipient == null) return;
            excipient.StrengthUnitDesc = excipient.StrengthUnitCd != null && _uomsLkp.ContainsKey(excipient.StrengthUnitCd) ? _uomsLkp[excipient.StrengthUnitCd] : excipient.StrengthUnitDesc;
            excipient.IngredientName = excipient.IngredientCd != null && _ingredientsLkp.ContainsKey(excipient.IngredientCd) ? _ingredientsLkp[excipient.IngredientCd] : excipient.IngredientName;
        }

        private void UpdateFormularyRoutesDMDLookup(FormularyRouteDetail routeDetail)
        {
            if (routeDetail == null) return;

            routeDetail.RouteDesc = routeDetail.RouteCd.IsEmpty() || !_routesLkp.IsCollectionValid() || !_routesLkp.ContainsKey(routeDetail.RouteCd) ? routeDetail.RouteDesc :
                _routesLkp[routeDetail.RouteCd];
        }

        private void UpdateFormularyLocalRoutesDMDLookup(FormularyLocalRouteDetail routeDetail)
        {
            if (routeDetail == null) return;

            routeDetail.RouteDesc = routeDetail.RouteCd.IsEmpty() || !_routesLkp.IsCollectionValid() || !_routesLkp.ContainsKey(routeDetail.RouteCd) ? routeDetail.RouteDesc : _routesLkp[routeDetail.RouteCd];
        }

        private void UpdateFormularyIngredientsDMDLookup(FormularyIngredient ingredient)
        {
            if (ingredient == null) return;

            ingredient.BasisOfPharmaceuticalStrengthDesc = ingredient.BasisOfPharmaceuticalStrengthCd.IsEmpty() || !_strengthsLkp.IsCollectionValid() || (_strengthsLkp.ContainsKey(ingredient.BasisOfPharmaceuticalStrengthCd) == false) ? ingredient.BasisOfPharmaceuticalStrengthDesc : _strengthsLkp[ingredient.BasisOfPharmaceuticalStrengthCd];

            ingredient.IngredientName = ingredient.IngredientCd.IsEmpty() || !_ingredientsLkp.IsCollectionValid() || (_ingredientsLkp?.ContainsKey(ingredient.IngredientCd) == false) ? ingredient.IngredientName : _ingredientsLkp[ingredient.IngredientCd];

            ingredient.StrengthValueNumeratorUnitDesc = ingredient.StrengthValueNumeratorUnitCd.IsEmpty() || !_uomsLkp.IsCollectionValid() || (_uomsLkp?.ContainsKey(ingredient.StrengthValueNumeratorUnitCd) == false) ? ingredient.StrengthValueNumeratorUnitDesc : _uomsLkp[ingredient.StrengthValueNumeratorUnitCd];

            ingredient.StrengthValueDenominatorUnitDesc = ingredient.StrengthValueDenominatorUnitCd.IsEmpty() || !_uomsLkp.IsCollectionValid() || (_uomsLkp?.ContainsKey(ingredient.StrengthValueDenominatorUnitCd) == false) ? ingredient.StrengthValueDenominatorUnitDesc : _uomsLkp[ingredient.StrengthValueDenominatorUnitCd];
        }
