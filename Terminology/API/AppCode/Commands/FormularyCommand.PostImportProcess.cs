...
namespace Interneuron.Terminology.API.AppCode.Commands
{
    public partial class FormularyCommand : IFormularyCommands
    {
        ...
        private void PopulateFormularyIngredientsForAMPFromVMP(FormularyHeader amp, FormularyHeader vmp)
        {
            ...
            amp.FormularyIngredient?.Each(rec => _formularyIngredientRepository?.Remove(rec));
        ...
        private void PopulateFormularyAdditionalCodesForAMPFromVMP(FormularyHeader amp, FormularyHeader vmp)
        {
            ...
            if (amp.FormularyAdditionalCode.IsCollectionValid())
            {
                ...
                amp.FormularyAdditionalCode.Where(rec => string.Compare(rec.AdditionalCodeSystem, "bnf", true) == 0 && rec.AdditionalCode != null)?
                    .Each(rec =>
                    {
                        if (!ampBNFs.Contains(rec.AdditionalCode)) ampBNFs.Add(rec.AdditionalCode);

                        var sevenCharCode = rec.AdditionalCode.Length >= 7 ? rec.AdditionalCode.Substring(0, 7) : rec.AdditionalCode;// null;

                        if (sevenCharCode != null && !ampBNFsSevenChars.Contains(sevenCharCode)) ampBNFsSevenChars.Add(sevenCharCode);
                    });

                //Added to delete the unmapped (null fk records in additinal codes table)
                ampAtcs?.Each(rec => _formularyAdditionalCodeRepository?.Remove(rec));
            ...
            if (vmp.FormularyAdditionalCode.IsCollectionValid())
            {
                var vmpAtcBnfs = vmp.FormularyAdditionalCode.Where(rec => string.Compare(rec.AdditionalCodeSystem, "bnf", true) == 0 || string.Compare(rec.AdditionalCodeSystem, "atc", true) == 0);

                if (vmpAtcBnfs.IsCollectionValid())
                {
                    foreach (var vmpAtcBNF in vmpAtcBnfs)
                    {
                        var sevenCharBNFFRomVMP = vmpAtcBNF.AdditionalCode != null && vmpAtcBNF.AdditionalCode.Length >= 7 ? vmpAtcBNF.AdditionalCode.Substring(0, 7) : vmpAtcBNF.AdditionalCode;
