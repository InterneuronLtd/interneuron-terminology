...
namespace Interneuron.Terminology.API.Controllers
{
    public partial class FormularyController : ControllerBase
    {
        ...
        public async Task<ActionResult> InvokePostImportProcess(List<string> codes)
        {
            ...
            {
                await this._formularyCommand.InvokePostImportProcessForCodes(codes, importedCodes =>
                {
                    if (importedCodes == null) return;

                    var cacheHandler = this._provider.GetService(typeof(CacheHandlerService)) as CacheHandlerService;

                    if (importedCodes != null && importedCodes.Count > 0)
                    {
                        cacheHandler.CacheActiveFormularyDetailRuleBound(importedCodes);
                    }
                });
            }
            else
                await this._formularyCommand.InvokePostImportProcess(() =>
                {
                    ...
                    cacheHandler.CacheActiveFormularyDetailRuleBound(null, true);
        ...
        public async Task<ActionResult> SyncAllPendingDMD()
        {
            ...
            await _formularyCommand.InvokePostImportProcessForCodes(codes, importedCodes =>
            {
                if (importedCodes == null) return;
