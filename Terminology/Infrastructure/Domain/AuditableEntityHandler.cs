		...
		public void Execute(EntityBase entity, EventType eventType)
		{
			...
			if (auditedEntity != null)
			{
				if (eventType == EventType.Add)
				{
					...
					auditedEntity.Createddate = DateTime.SpecifyKind(DateTime.Now, DateTimeKind.Local);
					auditedEntity.Createdtimestamp = DateTime.UtcNow;
					auditedEntity.Createdby = auditedEntity.Createdby ?? "System";

				}
				//auditedEntity.Updateddate = DateTime.UtcNow;
				auditedEntity.Updatedby = _requestContext?.TerminologyAPIUser?.UserId;

				auditedEntity.Updateddate = DateTime.SpecifyKind(DateTime.Now, DateTimeKind.Local);
				auditedEntity.Updatedtimestamp = DateTime.UtcNow;
				auditedEntity.Updatedby = auditedEntity.Updatedby ?? "System";
			}
		}
	}
